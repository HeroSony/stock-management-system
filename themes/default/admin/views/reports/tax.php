...
<script>
    $(document).ready(function () {
        oTable = $('#TAXData').dataTable({
            ...
            {"mRender": currencyFormat<?= $Settings->format_gst ? '' : ", 'bVisible': false"; ?>},
            {"mRender": currencyFormat<?= $Settings->format_gst ? '' : ", 'bVisible': false"; ?>},
            {"mRender": currencyFormat<?= $Settings->format_gst ? '' : ", 'bVisible': false"; ?>},
            {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat, "bVisible": false},
            {"mRender": row_status, 'bVisible': false}],
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                var gtotal = paid = ptax = otax = igst = cgst = sgst = 0;
                for (var i = 0; i < aaData.length; i++) {
                    igst += aaData[aiDisplay[i]][4] != null ? parseFloat(aaData[aiDisplay[i]][4]) : 0;
                    cgst += aaData[aiDisplay[i]][5] != null ? parseFloat(aaData[aiDisplay[i]][5]) : 0;
                    sgst += aaData[aiDisplay[i]][6] != null ? parseFloat(aaData[aiDisplay[i]][6]) : 0;
                    ptax += parseFloat(aaData[aiDisplay[i]][7]);
                    otax += parseFloat(aaData[aiDisplay[i]][8]);
                    gtotal += parseFloat(aaData[aiDisplay[i]][9]);
                    // paid += parseFloat(aaData[aiDisplay[i]][9]);
                }
                var nCells = nRow.getElementsByTagName('th');
                nCells[4].innerHTML = currencyFormat(parseFloat(igst));
                nCells[5].innerHTML = currencyFormat(parseFloat(cgst));
                nCells[6].innerHTML = currencyFormat(parseFloat(sgst));
                nCells[7].innerHTML = currencyFormat(parseFloat(ptax));
                nCells[8].innerHTML = currencyFormat(parseFloat(otax));
                ...
                <div class="table-responsive">
                    <table id="TAXData"
                        ...
                        <tr>
                            ...
                            <th style="min-width:120px !important;"><?= lang("paid"); ?></th>
                            <th style="width:80px !important;"><?= lang("payment_status"); ?></th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
    ...
    <script>
        $(document).ready(function () {
            oTable = $('#PTAXData').dataTable({
                ...
                'fnRowCallback': function (nRow, aData, iDisplayIndex) {
                    nRow.id = aData[11];
                    nRow.className = (aData[9] > 0) ? "purchase_link" : "purchase_link warning";
                    return nRow;
                },
                "aoColumns": [{"mRender": fld}, null, null, null,
                {"mRender": currencyFormat<?= $Settings->format_gst ? '' : ", 'bVisible': false"; ?>},
                        ...
                        <div class="table-responsive">
                            <table id="PTAXData"
                                ...
                                <tr>
                                    <th style="min-width:150px !important;"><?= lang("date"); ?></th>
                                    <th style="min-width:150px !important;"><?= lang("reference_no"); ?></th>
                                    <th style="min-width:150px !important;"><?= lang("warehoue"); ?></th>
